...
class BranchAndBoundQeue(Queue):
...
#Ramificación y Acotación
class BranchAndBoundQeueWithSubest(Queue):
    ...
    def pop(self):
        n = lambda node: node.path_cost + self.problem.h(node)
        self.A = sorted(self.A, key = n, reverse = True)
        return self.A.pop()
